...
title = "Correlation Heatmap of Cervical Cancer Biopsy Dataset"
heatmap <- create_heatmap(title, data_clean_num)
...
```{r}
# Rename factor names in line with valid R variable naming for linear models to work
...
lr_model <- lr_model_train(train_X, train_y_renam, ctrl)
lr_model
```
...
### LDA Model
```{r}
lda_model <- lda_model_train(train_X, train_y_renam, ctrl)
lda_model
```
...
```{r}
glmn_model <- glmn_model_train(train_X, train_y_renam, ctrl)
glmn_model
```
...
```{r}
nsc_model <- nsc_model_train(train_X, train_y_renam, ctrl)
nsc_model
```
...
plot(roc_lr, type = "s", add = TRUE, col='purple', legacy.axes = TRUE)
plot(roc_lda, type = "s", add = TRUE, col='grey', legacy.axes = TRUE)
plot(roc_glmn, type = "s", add = TRUE, col='peach', legacy.axes = TRUE)
plot(roc_nsc, type = "s", add = TRUE, legacy.axes = TRUE)


legend("bottomright", legend=c("Neural Network", "Multivariate Adaptive Regression Splines (MARS)", "Support Vector Machine (svmRadial)", "Support Vector Machine (svmPoly)","K-Nearest Neighbor", "Random Forest", "Logistic Regression", "Linear Discriminant Analysis", "Penalized Logistic Regression", "Nearest Shrunken Centroids"), 
       col=c("red", "green","blue", "brown", "yellow", "orange", "purple","grey","peach","black"), lwd=2)
...
```{r}
plot(varImp(glmn_model, scale = FALSE), top = 10,
     main = "Important Factors for Predicting Cervical Cancer \nBiopsies using Penalized Logistic Regression")

```

## Recursive Feature Elimination for Best Model (Penalized Logistic Regression)
```{r}
# use caret package & user-defined-function in Modeling.R to do recursive feature elimination
train_y_num <- as.numeric(train_y)
optimal_glm_features <- linear_rfe(train_X, train_y_num)
print(optimal_glm_features)
```


```{r}
# Retrain penalized LR with optimal features - 12 out of 15
train_X_rfe <- train_X[, optimal_glm_features]

glmn_model_rfe <- glmn_model_train(train_X_rfe, train_y_renam, ctrl)
glmn_model_rfe
```

```{r}
# Test new model
test_X_rfe <- test_X[, optimal_glm_features]
# get prediction result
testResults_glmn_rfe <- get_prediction_results(glmn_model_rfe, test_X_rfe, test_y)
testResults_glmn_rfe

# confusion matrix
cm <- confusionMatrix(as.factor(testResults_glmn_rfe$prediction), as.factor(testResults_glmn_rfe$observation))
print(cm)

# roc/auc result
roc_glmn_rfe <- roc(testResults_glmn_rfe$observation, testResults_glmn_rfe$class_prob)
auc(roc_glmn_rfe)
plot(roc_glmn_rfe)
```
...
plot(varImp(glmn_model_rfe, scale = FALSE), top = 10,
     main = "Important Factors for Predicting Cervical Cancer \nBiopsies using Penalized Logistic Regression")
